...
public class UTF8Converter {
    public static byte[] codePointToUTF(int x) {
        ...
        if (x < 1 << 7) {
            ...
            b = new byte[2];
            b[0] = (byte) ((0b1100_0000) | (x >>> 6));
            b[1] = (byte) ((0b1000_0000) | (x & 0b0011_1111));
        } else if (x < 1 << 16) {
            b = new byte[3];
            b[0] = (byte) ((0b1110_0000) | (x >>> 12));
            b[1] = (byte) ((0b1000_0000) | ((x >>> 6) & 0b0011_1111));
            b[2] = (byte) ((0b1000_0000) | (x & 0b0011_1111));
        } else {
            b = new byte[4];
            b[0] = (byte) ((0b1111_0000) | (x >>> 18));
            b[1] = (byte) ((0b1000_0000) | ((x >>> 12) & 0b0011_1111));
            b[2] = (byte) ((0b1000_0000) | ((x >>> 6) & 0b0011_1111));
            b[3] = (byte) ((0b1000_0000) | (x & 0b0011_1111));
        }
    ...
    public static int UTFtoCodePoint(byte[] bytes) {
        if (isValidUTF8(bytes)) {
            ...
            if (bytes.length == 1) {
                ...
                x = bytes[0] & 0b0001_1111;
            } else if (bytes.length == 3) {
                x = bytes[0] & 0b0000_1111;
            } else {
                x = bytes[0] & 0b0000_0111;
            }
            for (int i = 1; i < bytes.length; i++) {
                x = (x << 6) | (bytes[i] & 0b0011_11111);
            }
            return x;
        } else return 0;
    }
